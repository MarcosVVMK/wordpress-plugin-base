...
class HelperProvider {
	...
	public static function getVideoUri( $fileName ) {
		return plugin_dir_url( __FILE__ ) . "../../assets/videos/{$fileName}";
	}
    ...
    public static function CheckDateToShow()
    {
        $date = new DateTime( 'now' );
        $date->setTimezone( new DateTimeZone( 'America/Sao_Paulo' ) );
        $week_day       = $date->format( 'D' );
        $available_days = [ 'Mon', 'Tue', 'Wed', 'Thu', 'Fri' ];
        $hour           = intval( str_replace( ':', '', $date->format( 'G:i:s' ) ) );

        if ( in_array( $week_day, $available_days ) ) {
            return $hour > 80000 && $hour < 180000;
        }

        return $week_day == 'Sat' && ( $hour > 80000 && $hour < 120000 );
	}
}
